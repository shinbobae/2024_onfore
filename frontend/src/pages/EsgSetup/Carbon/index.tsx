...
import { CERTIFIED_OPTIONS } from '../data.ts';
...
import CarbonEditPopup from './CarbonEditPopup';
...
const EsgCarbon = () => {
  ...
  const [selectedColumn, setSelectedColumn] = useState<ESGSetupDataType[]>([]);

  const [tableData, setTableData] = useState<{ siteId: string; siteName: string; dataSource: ESGSetupDataType[] }[]>(
    [],
  );
  ...
  useEffect(() => {
    ...
    if (searchInput.length > 0) {
      for (let i = 0; i < searchedList.length; i++) {
        searchedList[i].dataSource = searchedList[i].dataSource.filter((item: ESGSetupDataType) =>
          item.boardName.toLowerCase().includes(searchInput.toLowerCase()),
        );
      }
    }
    if (certifiedFilter.length > 0) {
      //필터걸어
      for (let i = 0; i < searchedList.length; i++) {
        searchedList[i].dataSource = searchedList[i].dataSource.filter((item: ESGSetupDataType) =>
          certifiedFilter.includes(item.certifiedBy),
        );
      }
    }
    setTableData(searchedList);
  }, [esgTypeDataSource, certifiedFilter, searchInput]);

  const onChangeFilter = useCallback(
    (value: string) => {
      let newValue: string[] = [];
      if (certifiedFilter.find((role) => role === value)) {
        newValue = [...certifiedFilter].filter((role) => role !== value);
      } else {
        newValue = [...certifiedFilter, value];
      }
  ...
  const onDelete = useCallback(() => {
    deleteEsgTypeData({ esgType: 'carbon', id_list: selectedColumn.map((item) => item.itemId) });
  }, [selectedColumn]);
